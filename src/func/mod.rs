...
impl<'a> FunctionCompiler<'a> {
                            ...
                            match self.cpl.type_provider.get_typedef_by_name(&abs_obj_type) {
                                ...
                                None => {
                                    match self.cpl.type_provider.get_enum_by_name(&abs_obj_type) {
                                        Some(enum_type) => return Ok(enum_type.as_complex_type(self.cpl.type_provider.get_source_enum(&enum_type))),
                                        None => {
    ...
    fn resolve_class_member_ptr(
        ...
        let getter_impl = 'getter_block: {
            ...
            return Err(compiler_error!(self, "No such field or `get` accessor `{}` in type `{}`", field_name.token.0, type_root.full_name,));
        };
