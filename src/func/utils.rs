...
impl<'a> FunctionCompilerUtils for FunctionCompiler<'a> {
    ...
    fn get_array_element_ptr(
...

        {
            ...
            self.emit(Insn::Call(
                throw_out_of_bounds_ref,
                throw_out_of_bounds.as_llvm_type(&self.cpl),
                vec![index.val, length],
            ));
                    ...
                    ClassType::Struct => {
                        ...
                        for i in 0..class.fields.len() as u32 {
                            ...
                            self.copy(
                                ...
                                &TypedValue::new(field_ty, dest_element_ptr),
                            )?;
                        }
                    }
                    _ => {
    ...
    fn handle_unhandled_error(&mut self, check: bool) -> Result<()> {
        ...
        if self.state.block_stack.len() == 0 {
            return Ok(());
        }
        ...
        if check {
            let has_unhandled_error_callable = ResolvedFunctionNode::externed(
                "keid.has_unhandled_error",
                &[],
                Varargs::None,
                BasicType::Bool.to_complex(),
            );
            let has_unhandled_error_ref = self.get_function_ref(&has_unhandled_error_callable)?;
            let has_unhandled_error_val =
                self.call_function(has_unhandled_error_ref, &has_unhandled_error_callable, &[])?;
    ...
    fn return_null(&mut self) {
        if self.func.return_type == BasicType::Void.to_complex()
            || self.func.return_type.is_struct(&self.cpl.type_provider)
        {
...
pub fn get_import_map_with(
    ...
    for node in imports {
        let types: Vec<String> = all_type_names
            .iter()
            .filter(|type_name| get_type_namespace(&type_name) == node.module)
