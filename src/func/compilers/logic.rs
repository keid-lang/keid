...
impl<'a> LogicCompiler for FunctionCompiler<'a> {
    fn compile_cast(&mut self, from: TypedValue, to: ComplexType) -> Result<TypedValue> {
        let casted = match (&from.ty, &to) {
            (ComplexType::Basic(original), ComplexType::Basic(target)) => {
                match (original, target) {
                    (is_int!(), is_int!()) => self.emit(Insn::IntCast(
                        from.val,
                        target.as_llvm_type(self.cpl),
                        original.is_signed(),
                    )),
                    (is_int!(), is_float!()) => self.emit(Insn::IntToFloat(
                        from.val,
                        target.as_llvm_type(self.cpl),
                        original.is_signed(),
                    )),
                    (is_float!(), is_int!()) => self.emit(Insn::FloatToInt(
                        from.val,
                        target.as_llvm_type(self.cpl),
                        target.is_signed(),
                    )),
                    (is_float!(), is_float!()) => {
                        self.emit(Insn::FloatCast(from.val, target.as_llvm_type(self.cpl)))
                    }
                    _ => {
    ...
    fn compile_logic_expr(
        ...
        match (&lhs.ty, &rhs.ty) {
                ...
                let array_equals_impl = self
                    ...
                    .get_function_by_name(
                        &GenericIdentifier::from_name_with_args(
                            "core::array::equals",
                            &[*lhs_element_type.clone()],
                        ),
                        &[
                            (*lhs_element_type.clone()).to_array(),
                            (*lhs_element_type.clone()).to_array(),
                        ],
                    )
                    .unwrap();
            ...
            (
                ComplexType::Basic(BasicType::Object(lhs_ident)),
                ComplexType::Basic(BasicType::Object(rhs_ident)),
            ) => {
                ...
                let resolved_interface_impls = self
                    .cpl
                    .type_provider
                    .get_resolved_interface_impls(lhs_ident);
                for resolved_interface_impl in resolved_interface_impls {
                    let source_interface_impl = self
                        .cpl
                        .type_provider
                        ...
                        && resolved_interface_impl.interface_generic_impls[0] == rhs_type
                    {
                        ...
                        let callable = self
                            ...
                            .unwrap();
                        let callable =
                            self.resolve_interface_impl_function(callable, source_interface_impl)?;
                        ...
                        let result =
                            self.call_function(method_ptr, &callable, &[lhs.clone(), rhs.clone()])?;
        ...
        let is_signed = if matches!(&lhs.ty, ComplexType::Basic(is_int!()))
            && matches!(&rhs.ty, ComplexType::Basic(is_int!()))
        {
            let lhs_signed = matches!(
                &lhs.ty,
                ComplexType::Basic(
                    BasicType::Int8 | BasicType::Int16 | BasicType::Int32 | BasicType::Int64
                )
            );
            let rhs_signed = matches!(
                &rhs.ty,
                ComplexType::Basic(
                    BasicType::Int8 | BasicType::Int16 | BasicType::Int32 | BasicType::Int64
                )
            ...
            if rhs_signed != lhs_signed {
                return Err(compiler_error!(
                    self,
                    "Cannot compare integers of type `{}` and `{}`",
                    lhs.ty.to_string(),
                    rhs.ty.to_string()
                ));
            }
        ...
        let (val, ty) = match &op {
            Operator::Equals | Operator::NotEquals => {
                ...
                let (lhs_val, rhs_val) = match ((lhs.val, &lhs.ty), (rhs.val, &rhs.ty)) {
                    ...
                    (
                        (nullable, ComplexType::Nullable(inner)),
                        (_, ComplexType::Basic(BasicType::Null)),
                    )
                    | (
                        (_, ComplexType::Basic(BasicType::Null)),
                        (nullable, ComplexType::Nullable(inner)),
