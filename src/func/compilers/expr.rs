...
impl<'a> ExprCompiler for FunctionCompiler<'a> {
    ...
    fn compile_expr(&mut self, expr: &Token<Expr>, type_hint: Option<&ComplexType>) -> Result<TypedValue> {
        ...
        Ok(match &expr.token {
            ...
            Expr::CharLit(ch) => TypedValue::new(BasicType::Char.to_complex(), self.cpl.context.const_int(self.cpl.context.get_i64_type(), *ch as u64)),
            Expr::Ident(ident) => {
    ...
    fn compile_match_expr(&mut self, mtch: &MatchExpr) -> Result<TypedValue> {
        ...
        println!("Start {:#?}", self.state.block_stack);
        ...
        println!("End {:#?}", self.state.block_stack);
