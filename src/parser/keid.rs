            ...
            _ => {
                return Err(Error::<Rule>::new_from_span(
                    ErrorVariant::CustomError {
                        message: "Let statement must have either an explicit type or an initial value, or both"
                            .to_string(),
                    },
                    name_pair.as_span(),
                )
...
fn parse_indefinite_loop(mut pairs: Pairs<Rule>) -> Result<Vec<Token<Statement>>> {
    ...
    Ok(parse_block(pairs.next().unwrap())?)
}
...
fn parse_class_decl(mut pairs: Pairs<Rule>, namespace: Qualifier, ty: ClassType) -> Result<ClassDecl> {
    ...
    for class_statement in class_block {
        match class_statement.as_rule() {
            ...
            Rule::get_accessor_decl | Rule::set_accessor_decl => {
                accessors.push(parse_accessor_decl(class_statement.into_inner())?)
            }
            Rule::interface_accessor_decl => {
                accessors.extend(parse_interface_accessor_decl(class_statement.into_inner())?)
            }
            Rule::associated_type_decl => {
                associated_type_names.push(parse_associated_type(class_statement.into_inner())?)
            }
            x => unreachable!("{:?}", x),
        }
    }
