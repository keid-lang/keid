...
impl Linker {
    ...
    fn get_gcc_search_dirs(&self) -> Result<Vec<String>> {
        let gcc_output = String::from_utf8(
            Command::new("gcc")
    ...
    fn get_system_dynamic_linker(&self) -> Result<String> {
        for dir in self.get_gcc_search_dirs()? {
    ...
    pub fn link<T: Into<std::path::PathBuf>>(&self, target: ObjectFormat, path: T) {
        use llvm_sys::linker::*;
        use std::ffi::CString;
        ...
        args.push(
            self.get_system_dynamic_linker()
                .expect("no system linker found"),
        );
        args.push(format!("{}/crt1.o", crt_dir));
        args.push(format!("{}/crti.o", crt_dir));
        args.push("-o".to_owned());
        args.push(path.join("exec").as_os_str().to_str().unwrap().to_owned());
        args.push("-e".to_owned());
        args.push("_start".to_owned());
        args.push("-L/usr/lib".to_owned());
        args.extend(
            self.get_gcc_search_dirs()
                .expect("getting gcc search dirs")
                .into_iter()
        ...
        args.push(format!("-L{}", path.as_os_str().to_str().unwrap().to_owned()));
        ...
        unsafe {
            let args_slice: Vec<CString> = args
                .into_iter()
                .map(|str| CString::new(str).expect("invalid arg"))
                .collect();
